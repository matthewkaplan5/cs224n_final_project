...
class QANetOutput(nn.Module):
    """
    The output layer in QANet

    We concatenate inputs, then apply a linear later than apply a softmax to generate log probabilities.

    """
    def __init__(self, input_size):
        super(QANetOutput, self).__init__()
        # From paper: w1 and w1 matrices concatenated inputs that we want to send to 1
        # (to go from 3 to 2 dimensions)
        self.w1 = nn.Linear(in_features = 2*input_size, out_features = 1)
        self.w2 = nn.Linear(in_features = 2*input_size, out_features = 1)

    def forward(self, m0, m1, m2, mask):
        # M0, M1, and M2 all have shapes (batch_size, seq_len, input_size)
        ...
        log_p1 = masked_softmax(x1.squeeze(), mask, log_softmax=True) # squeeze from 3 to 2 dims
        log_p1 = masked_softmax(x2.squeeze(), mask, log_softmax=True)
